namespace T;

entity RATES{
	key YEAR			: Integer;
	key	MONTH			: String(3);
	key	FWD_MONTH		: Integer;
		AVG_SPOT_RATE	: Decimal(25, 20);
		QUOTE_PRICE		: Decimal(25, 20);
}

entity PROFIT_LOSS_ALL{
	key YEAR			: Integer;
	key FIRST_MONTH_START   : String(20);
    key FIRST_MONTH_END     : String(20);
    key SECOND_MONTH_START  : String(20);
    key SECOND_MONTH_END    : String(20);
    FIRST_PROFIT_LOSS   : Decimal(25, 20);
    SECOND_PROFIT_LOSS  : Decimal(25, 20);
    
}

entity LAYER_YEAR_PNL{
    key SMART_KEY : String(20);
    key SIMULATION_ID : Integer;
    key YEAR : Integer;
    key LAYER : Integer;
    MONTH_START : String(20);
    MONTH_START_ID : Integer;
    MONTH_START_DATE : Date;
    MONTH_END : String(20);
    MONTH_END_ID : Integer;
    FORWARD_MONTH : Integer;
    MONTH_START_KEY : String(20);
    MONTH_END_DATE : Date;
    MONTH_END_KEY : String(20);
    FORWARD_PRICE : Decimal(25, 20);
    SPOT_PRICE : Decimal(25, 20);
    LAYER_PNL : Decimal(25, 20);

} 